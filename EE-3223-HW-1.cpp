...
int main() {
	...
	cout << "Input a value for Vdc in volts" << endl;
	cin >> voltage_Source;
	

	// while loop to check and see if input matches the var so no exceptions are caught.
	// i know this is not required, but habit from a java class i took years ago
	while (!cin) {
	...
	cout << "Input a value for resistor 1 in ohms" << endl;
	...
	cout << "Input a value for resistor 2 in ohms" << endl;
	cin >> resist_2;
	...
	cout << "The voltage divider value in volts is " << volt_Between << "volts" << endl;
